...
export async function initCommand(): Promise<void> {
  ...
  try {
    ...
    if (existsSync(configPath)) {
      ...
      console.log(chalk.blue('To overwrite, please remove the existing .reviewai.yaml file first.'));
      return;
    }
    ...
    console.log('\nNext steps:');
    console.log(chalk.blue('1. Set your OpenAI API key in .env or as environment variable'));
    console.log(chalk.blue('2. Customize the configuration in .reviewai.yaml'));
    console.log(chalk.blue('3. Run `reviewai review` to start reviewing code\n'));
  } catch (error) {
